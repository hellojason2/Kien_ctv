#!/usr/bin/env python3
"""
Import Nha Khoa (Dental) Customer Services TSV file
- Each row = one service record
- Customers deduplicated by phone number
- CTV accounts created from Column K (Người chốt) if they don't exist
...
def main():
    print("=" * 60)
    print("Import Nha Khoa (Dental) Customer Services")
    print("=" * 60)
    ...
    with open(TSV_FILE, 'r', encoding='utf-8') as f:
        ...
        for row in reader:
            rows.append(row)
            ctv_code = row.get('Người chốt', '').strip()
            if ctv_code:
                ctv_codes.add(ctv_code)
    
    print(f"   Found {len(rows)} service records")
    print(f"   Found {len(ctv_codes)} unique CTV codes (Người chốt)")
    print(f"   CTV codes: {', '.join(sorted(ctv_codes))}")
    ...
    try:
        ...
        print(f"   ✓ Found {customers_existing} existing customers")
        ...
        for row in rows:
            ...
            ctv_code = row.get('Người chốt', '').strip() or None
            service_name = row.get('Dịch vụ làm', '').strip()[:200]
            date_entered = parse_date(row.get('Ngày nhập đơn', ''))
            date_scheduled = parse_date(row.get('Ngày làm', ''))
            tong_tien = parse_money(row.get('Giá tổng đơn', ''))
            
            cur.execute("""
                INSERT INTO services 
                (customer_id, service_name, date_entered, date_scheduled, ctv_code, tong_tien, status)
                VALUES (%s, %s, %s, %s, %s, %s, 'Cho xu ly')
            """, (customer_id, service_name, date_entered, date_scheduled, ctv_code, tong_tien))
            services_created += 1
